<link href="modules/addons/solutedns/template/css/bootstrap.css" rel="stylesheet" type="text/css" />
...
<div class="bt3">
	...
	<div class="row">
		<div class="col-md-3 sidebar">
			<div class="panel panel-default panel-actions">
				...
				<div class="list-group">
					<a href="#overview" id="overview-tab" class="list-group-item active" onclick="setOverview('overview');" aria-controls="overview" role="tab" data-toggle="tab">{$MLANG.client_overview}</a> {if $lal >= 3}<a href="#template" id="template-tab" class="list-group-item {if $client_template eq false}disabled{/if}" {if $client_template eq true} onclick="setOverview('template'); drawRecords('sdns_template');" {/if} aria-controls="template" role="tab" data-toggle="tab">{$MLANG.client_defaulttemplate}</a>{/if} {if $lal >= 3}<a href="#reverse" id="reverse-tab" class="list-group-item {if $reverse_management eq false}disabled{/if}" {if $reverse_management eq true} onclick="setOverview('reverse'); drawReverse('sdns_reverse');" {/if} aria-controls="reverse" role="tab" data-toggle="tab">{$MLANG.client_reversedns}</a>{/if}
				</div>
			</div>
			{if $maintenance eq false} {if $client_zones eq true} {if $lal >= 3}
			...
			<div class="text-center"><small><label for="add_zone">{$MLANG.client_addupto} {$zone_limit} {$MLANG.client_zones}.</label></small>
			</div>
			{/if}
            {/if}
            {/if}
		...
		<div class="col-md-9">
			...
			<div class="tab-content">
				...
				{if $lal >= 3} {if $client_template eq true}
				<!-- Template Tab -->
				<div role="tabpanel" class="tab-pane" id="template">
					...
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							<div class="text-center"><a href="#" onclick="activate_template();" class="btn btn-lg btn-success">{$MLANG.client_activate}</a>
							</div>
						</div>
					</div>
					{/if}
	...
	{if $maintenance eq false} {if $client_zones eq true || $client_template eq true}
	<!-- Add Modal -->
	<div class="bootstrap">
